...
const EditOrderScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        ...
        const clientiList = clientiSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
        ...
        const prodottiList = prodottiSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      Toast.show({
        type: "success",
        text1: "Modifiche Salvate!",
        text2: "L'ordine è stato aggiornato.",
      });
      navigation.goBack();
    } catch (error) {
      ...
      Toast.show({
        type: "error",
        text1: "Errore",
        text2: "Impossibile salvare le modifiche.",
      });
    }
    setLoading(false);
  };
  ...
  useEffect(() => {
    setFilteredClienti(
      clienti.filter((cliente) => cliente.nome.toLowerCase().includes(searchCliente.toLowerCase()))
    );
  }, [searchCliente]);
  ...
  useEffect(() => {
    setFilteredProdotti(
      prodotti.filter((prodotto) =>
        prodotto.nome.toLowerCase().includes(searchProdotto.toLowerCase())
      )
    );
  }, [searchProdotto]);
